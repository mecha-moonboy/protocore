...
creatura.register_utility("animalia:wander", function(self)
	...
	if not center then return end -- if center is nil, return/break
	local function func(_self)
		if check_timer then
			check_timer = (check_timer <= 0 and 5) or check_timer - _self.dtime
		end
		if not _self:get_action() then -- if this entity does not have an action
			local dir
			local pos = _self.object:get_pos()
...
creatura.register_utility("animalia:eat_turf", function(self)
	...
	local function func(_self)
		local pos = _self.object:get_pos()
		if not pos then return end -- always check if pos is nil?
		local yaw = _self.object:get_yaw()
		...
		if not _self:get_action() then -- if there is no current action
