...
-- dialogue
dialogue = {}
function dialogue.get_formspec(name)
...
local function find_tree(self)
	...
	local treepos = minetest.find_node_near(pos, 32, {"group:tree"} )
	return treepos
end

local function chop_tree(self, pos)
...
local function shared_owner(obj1, obj2) -- check if entities have the same owner
	if not obj1 or not obj2 then return false end -- if either given parameter is false, break
	...
	and obj2:get_luaentity() then -- if they all are non-nil and have an attached luaentity
...
creatura.register_mob("friends_foes:individual", {
	-- Engine Props
	visual_size = {x = 1, y = 1},
	mesh = "friends_foes_individual.obj",
	textures = {
		"friends_foes_individual_1.png",
		"friends_foes_individual_2.png",
		"friends_foes_individual_3.png",
		"friends_foes_individual_4.png"
	},
	...
	utility_stack = {
		...
		},
		{
			...
			get_score = function(self)
				...
				if self:get_utility() == "animalia:walk_to_pos_and_interact" then
					return 0.1, {self, find_tree, chop_tree, "chop"}
				end
		...
		-- {
		-- 	utility = "friends_foes:chop_tree",
		-- 	step_delay = 0.25,
		-- 	get_score = function(self)
		-- 		return 0.7, {self}
		-- 	end
	...
	on_rightclick = function(self, clicker)
		...
		local passive = true
		minetest.show_formspec(name, "friends_foes:dialogue", dialogue.get_formspec(name))
...
creatura.register_spawn_item("friends_foes:individual", {
	col1 = "0092ff",
	col2 = "783f04"
})